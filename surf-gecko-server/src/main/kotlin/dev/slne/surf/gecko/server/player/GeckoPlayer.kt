...
import dev.slne.surf.gecko.server.chat.BoundChatType
import dev.slne.surf.gecko.server.chat.GeckoChatTypes
import dev.slne.surf.gecko.server.chat.OutgoingChatMessage
import dev.slne.surf.gecko.server.chat.PlayerChatHandler
import dev.slne.surf.gecko.server.chat.signature.FILTER_MASK_PASS_THROUGH
...
import dev.slne.surf.gecko.server.chat.signature.SignedMessageLink
...
class GeckoPlayer @AssistedInject constructor(
    ...
    companion object {
        private val POINTERS_SUPPLIER = PointersSupplier.builder<GeckoPlayer>()
            .parent(PLAYER_POINTERS_SUPPLIER)
            .resolving(PermissionChecker.POINTER, GeckoPlayer::permissionChecker)
            .build()
    }
    ...
    override fun sendSignedMessage(
        ...
        OutgoingChatMessage.create(signed).sendToPlayer(
            player = this,
            filtered = false,
            chatType = BoundChatType(GeckoChatTypes.raw, boundName),
    ...
    override fun sendRemoteSignedMessage(
        ...
        chatHandler.sendPlayerChatMessage(signed, BoundChatType(GeckoChatTypes.raw, boundName))

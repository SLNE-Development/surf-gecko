...
class PlayerChatHandler(
    ...
    private val config: Config.ChatConfig,
) : AutoCloseable {
    companion object {
    ...
    private val chatMessageChain = ChatMessageChain(geckoBlockingScope)
    ...
    private fun getSignedArguments(
        ...
        val signedArguments = Object2ObjectOpenHashMap<String, SignedMessage>(entries.size)
        entries.forEach { entry ->
            ...
            try {
                signedArguments[entry.name()] =
                    signedMessageDecoder.unpack(entry.signature(), body).adventureView()
            } catch (failure: SignedMessageChain.DecodeException) {
                handleMessageDecodeFailure(failure)
                return null
            }

...
object PluginDependencyResolver {
    ...
    fun resolve(plugins: Collection<MinestomPlugin>): List<MinestomPlugin> {
        val pluginsById = plugins.groupBy { plugin -> plugin.meta.id }
        ...
        val byId = pluginsById.mapValues { (_, pluginsForId) -> pluginsForId.single() }
        ...
        for (plugin in plugins) {
            for (dependency in plugin.meta.dependsOn) {
                dependents.getOrPut(dependency, ::linkedSetOf).add(plugin.meta.id)
            }
        }
        ...
        if (result.size != plugins.size) {
            val unresolved = remainingDependencies
                .filterValues { dependencies -> dependencies.isNotEmpty() }
                .entries
                .sortedBy { (id, _) -> id }
                .joinToString { (id, dependencies) -> "$id -> ${dependencies.sorted()}" }

...
import dev.slne.surf.gecko.command.platform.brigadier.SignedMessageArgumentType
import dev.slne.surf.gecko.command.platform.brigadier.SuggestingArgumentType
...
object SignedCommandArguments {
    ...
    fun <T> withMessages(messages: Map<String, SignedMessage>, block: () -> T): T {
    ...
    fun find(nodeName: String): SignedMessage? = current.get()?.get(nodeName)
}
